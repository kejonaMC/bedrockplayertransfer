pipeline {
    ...
    stages {
        ...
        stage('Post') {
            steps {
                archiveArtifacts 'bungeecord/target/*.jar,spigot/target/*.jar,velocity/target/*.jar', excludes: 'bungeecord/target/original-*.jar, spigot/target/original-*.jar, velocity/target/original-*.jar', fingerprint: true
